...
export class EncryptFunctions {
     ...
     */
    static encryptBuffer(str: string | Buffer, encryptionKey: string): Buffer<ArrayBuffer> {
        const keyBuffer = Buffer.from(encryptionKey, 'hex');
        const iv = crypto.randomBytes(16); // Crear un IV aleatorio
        const cipher = crypto.createCipheriv(EncryptFunctions.CYPHER_TO_USE, keyBuffer, iv);
     ...
     */
    static decryptBuffer(buffer: Buffer<ArrayBuffer>, encryptionKey: string): Buffer<ArrayBuffer> {
        const keyBuffer = Buffer.from(encryptionKey, 'hex');
        // Extraer el IV de los primeros 16 bytes del buffer
        const iv = buffer.subarray(0, 16);
        const encryptedData = buffer.subarray(16);
        const decipher = crypto.createDecipheriv(EncryptFunctions.CYPHER_TO_USE, keyBuffer, iv);
