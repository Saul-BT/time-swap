...
export class ErrorManager extends Error {
     ...
     */
    constructor(
        public readonly type: keyof typeof HttpStatus | HttpStatus, // Limita el valor de `type` a solo las claves (nombres de estado) dentro del objeto `HttpStatus`, asegurando que solo se usen estados HTTP válidos (por ejemplo, `NOT_FOUND`, `BAD_REQUEST`).
    ...
    public throwSignatureError() {
        ...
        if (typeof this.type === 'string') {
            throw new HttpException(this.message, HttpStatus[this.type]);
        } else {
            throw new HttpException(this.message, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public static normalize(error: unknown, messageOpt?: string): never {
        ...
        if (error && typeof error === 'object' && ('status' in error || 'statusCode' in error)) {
            // Parsea status
            const errorObj = error as Record<string, unknown>;
            const statusUnk: unknown = 'status' in error ? errorObj['status'] : errorObj['statusCode'];
            ...
            // Sino mandar 500
            throw new ErrorManager(HttpStatus.INTERNAL_SERVER_ERROR, messageOpt || 'Error inesperado');
        }
    }
     ...
     * @return {ErrorManager} Error del tipo ErrorManager
     */
    public static normalizeRestrictive(error: unknown, messageOpt?: string): never {
        // Se lanza el error si es de tipo ErrorManager
        if (error instanceof ErrorManager) throw error;
