...
export class AuditLogService implements OnModuleInit {
    ...
    // Los dos tipos de
    private readonly propertiesToFilter = ['companyId', 'username', 'role', 'userId'];
    ...
    constructor(
        ...
        this.MAX_LOG_SIZE = parseInt(this.configService.getOrThrow<string>('MAX_LOG_FILE_SIZE'));
    ...
    buildLogKey(log: LogAudit): string {
        ...
        if (log.isValidJWT && log.JWTInfo) {
            ...
            let company = '';
            if (log.JWTInfo.companiesIds && log.JWTInfo.companiesIds.length === 1) {
     ...
     * @returns
     *
    parseLogKeys(key: string): string[] | null {
    ...
    onModuleInit() {
        ...
        const crontimeSaveFlush = this.configService.getOrThrow<string>('SAVE_FLUSH_LOGS_CRON');
        const saveFlushLogsCron = new CronJob(crontimeSaveFlush, this.flushLogsInitCron.bind(this));
        this.schedulerRegistry.addCronJob('SAVE_&_FLUSH_LOGS', saveFlushLogsCron);
            ...
            if ((await this.logs.size) > 0) {
                ...
                for (const [key, logs] of this.logs) {
                    ...
                    if (keyInter) {
                        ...
                        for (let i = this.filenamePropStartIdx; i < this.propertiesToFilter.length; i++) {
                            const k = keyInter[i];
    ...
    async saveLogs(path: string[], filename: string, logs: LogAudit[]) {
        const bucket = FileBuckets.AUDIT_LOGS;
        ...
        try {
            ...
            if (lastLog && lastLog.size < this.MAX_LOG_SIZE) {
                ...
                const logsToAdd = AuditLogService.divideUntilNoFit(logsStr, this.MAX_LOG_SIZE - lastLog.size);
                // Recoge el archivo, desencriptandolo (si activado)
                const file = await this.fileService.retrieveFile(lastLog.etag, bucket, path, this.ENCRYPT_LOGS);
                ...
                // Comprime el buffer
                const bufferGzip = await FileService.compressBuffer(Buffer.from(updatedContent, 'utf-8')); // Vuelve a comprimirlo
                ...
                if (logsToAdd.rest.length > 0) {
                    await this.addToNewLogFiles(logsToAdd.rest, bucket, path, filename, logFiles.length);
                }
            } else {
                // Añade un o varios nuevos archivos de logs
                await this.addToNewLogFiles(logsStr, bucket, path, filename, logFiles.length);
            }
        } catch (err) {
     ...
     * @returns
     *
    static divideToChunks(strs: string[], maxChunkSize: number = 1024 * 1024 * 4) {
        const chunks: string[][] = [];
     ...
     * @param nameOffset
     *
    async addToNewLogFiles(logs: string[], bucket: string, path: string[], fileName: string, nameOffset: number) {
        // Añade los logs a uno o varios archivos
        ...
        for (let i = 0; i < logChunks.length; i++) {
            ...
            const fileNumberSuffix = i + nameOffset > 0 ? '-' + String(i + nameOffset) : '';
